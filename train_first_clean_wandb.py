    ...
    if accelerator.is_main_process:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=wandb_config.get('name', f"first_stage_{datetime.now().strftime('%Y%m%d_%H%M%S')}"),
                group=wandb_config.get('group', 'basque_first_stage'),
                job_type="first_stage_training",
                notes=wandb_config.get('notes', 'First stage training'),
                tags=wandb_config.get('tags', ['first_stage', 'basque', 'StyleTTS']),
                config=config,
...
def main(config_path):
    ...
    with accelerator.main_process_first():
        ...
        # load BERT model
        # NOTE: if you want stricter runtime checks that ensure the imported
        # `util` comes from the configured `PLBERT_dir`, run the test copy
        # `train_first_clean_wandb_plbert_check.py` instead.
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if (i+1)%log_interval == 0 and accelerator.is_main_process:
                ...
                loss_data = {
                    ...
                    'train/mono_loss': float(loss_mono) if torch.is_tensor(loss_mono) else loss_mono,
                    'train/s2s_loss': float(loss_s2s) if torch.is_tensor(loss_s2s) else loss_s2s,
                    'train/slm_loss': float(loss_slm) if torch.is_tensor(loss_slm) else loss_slm,
                    'epoch': epoch,
                    'step': iters
                }
        ...
        if accelerator.is_main_process:
            ...
            # keep a single newline after the validation message to avoid huge blank blocks in logs
            ...
            with torch.no_grad():
                for bib in range(len(asr)):
                    ...
                    if epoch % 10 == 0:  # Log audio every 10 epochs
                        y_rec_np = y_rec.cpu().numpy().squeeze()
                        
                        audio_data = {
                            f"audio/generated_{bib}": wandb.Audio(y_rec_np, sample_rate=sr),
                        }
